...
p3 = plot!([labortaxopt[ind],0.08], seriestype="vline", xticks = ([labortaxopt[ind],0.08],["\$ {\\dfrac{Taxes}{GDP}}^{tr} = 0.113 \$", "\$ {\\dfrac{Taxes}{GDP}}^{p} = 0.08 \$" ]), linewidth = 1, dpi=200,linestyle =:dash ,  label="")
savefig(p3, "Welfare_optimal_low_tau_labor.png")

using LaTeXStrings
p4= plot(τlopt,Welfare,label="Welfare_transition", linewidth = 2, dpi=200, color = :blue)
#p4= plot!(tau, Welfare_steady ,label="Welfare_ss", linewidth = 2, dpi=200 ,legend=:bottomleft)
p4 = plot!([τlopt[ind],0.125], seriestype="vline", xticks = ([τlopt[ind],0.125],["\$ \\tau^{tr} = 0.204 \$", "\$ \\tau^{p} = 0.125 \$" ]), linewidth = 1, dpi=200,linestyle =:dash ,  label="")
