...
function PricesEGM(K,TT,τl,Z,params::AiyagariParametersEGM) 
...
function AiyagariEGM(
    ...
    AggVars = PricesEGM(K,TT,τl,1.0,params)
    @unpack R,w = AggVars
...
function steady(
    ...
    for kit = 1:maxn
        K0 = ((R0-1 + δ)/(α*1^(1-α)))^(1 /(α-1))
        Aggs = PricesEGM(K0,TT,τl,1.0,params) #steady state
        cmat .= 0.0
        ...
        println("ir: ",R0," supply ",EA," demand ",((R0-1 + δ)/(α*1^(1-α)))^(1 /(α-1)))
