...
function UpdateAggs(
    ...
    for i = TimePeriods:-1:2
        #println("time: ",i-1)
        K,Z_P,TT_P = Kguess[i],Zpath[i], TTpath[i] # K[i] is the productive capital in period i, it is K(t-1)
        K_m,Z, TT = Kguess[i-1],Zpath[i-1], TTpath[i-1]
        Pr_P  = PricesMIT(K,TT_P,Z_P,params) #tomorrow
...
function equilibrium(Kguess,Zpath,initialpolc,initialpolW,initialpol,initialdis,AiyagariModel,TTpath, σf,tol = 1e-10,maxn = 50)

    polc_ss,U_ss,pol_ss,dis_ss =initialpolc,initialpolW, initialpol,initialdis
    aggK = copy(Kguess)
    apols,cpols, devol,Utpols,aggK = UpdateAggs(polc_ss,U_ss ,pol_ss,dis_ss,Kguess,Zpath,AiyagariModel,TTpath,σf)
