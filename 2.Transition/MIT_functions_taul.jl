...
function UpdateAggs(
    ...
    for i = TimePeriods:-1:2
        ...
        Pr_P  = PricesMIT(K,TT,τl,Z_P,params) #tomorrow
        Pr    = PricesMIT(K_m,TT,τl,Z,params) #today
        cmat  = zeros(eltype(initialpol),na*ns)
        cmat2 = zeros(eltype(initialpol),na*ns)
...
function equilibrium(Kguess,Zpath,initialpolc,initialpolW,initialpol,initialdis,AiyagariModel,τlprime, σf,tol = 1e-10,maxn = 50)

    polc_ss,U_ss,pol_ss,dis_ss, τl_t =initialpolc,initialpolW, initialpol,initialdis, τlprime
    aggK = copy(Kguess)
    apols,cpols, devol,Utpols,aggK = UpdateAggs(polc_ss,U_ss ,pol_ss,dis_ss,Kguess,Zpath,AiyagariModel,τl_t,σf)
