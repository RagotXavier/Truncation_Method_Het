...
ap =80 #number of alternative policies
TTpath = TT_path[2:end]

TTpath_ap1 = 0.30233704025344266*ones(Float64,TimePeriods+1, ap) #trajectories for TT in the alternative policies
for i = 1: ap
    Spath  = 0.30233704025344266*ones(Float64,TimePeriods+1) #add time 0
    epsz =  -0.015005478259033256 -(i/1000) #standard deviation for the shock
    rhoz = 0.986988
    Spath[1]=0.30233704025344266+epsz
    σf = 0
    for i=2:TimePeriods
        Spath[i] = 0.30233704025344266+ rhoz*(Spath[i-1]-0.30233704025344266)
    end
    TTpath_ap1[:,i] = Spath
end
...
TTpath_ap2 = 0.30233704025344266*ones(Float64,TimePeriods+1, ap) #trajectories for TT in the alternative policies
for i = 1: ap
    Spath  = 0.30233704025344266*ones(Float64,TimePeriods+1) #add time 0
    epsz =  -0.015005478259033256 + (i/1000) #standard deviation for the shock
    rhoz = 0.986988
...
TTpath_r = 0.30233704025344266*ones(Float64,TimePeriods+1, ap) #trajectories for TT in the alternative policies
    Spath  = 0.30233704025344266*ones(Float64,TimePeriods+1) #add time 0
    epsz =  -0.015005478259033256  #standard deviation for the shock
    rhoz = 0.986988
    Spath[1]=0.30233704025344266+epsz
    σf = 0
    for i=2:TimePeriods
...
TTpath_ap = [TTpath_ap1 TTpath_r TTpath_ap2]
...
for i = 1: length(Welfare)
    Epsilon[i] = exp((1- β)*(Welfare[i] - Welfare[ap+1])) -1
end

plot(-ap:1:ap, 100 .*Epsilon,
xlabel="Percentage deviation from the optimal path", ylabel="Consumption equivalent", label = "")

100*(TTpath_ap[1,ind] - TTpath_ap[1,ap+1])./TTpath_ap[1,ap+1] #deviation in percentage terms in the taxes in period 1

tau_ind = TTpath_ap[1:end-1,ind]./Yv[:, ind] #optimal tax path in transition
tau_ap = TTpath_ap[1:end-1,ap+1]./Yv[:, ap+1] #optimal tax path in projection


index = ones(length(TTpath_ap[1,:]))

for i = 1:length(TTpath_ap[1,:])
    index[i] = 100*(TTpath_ap[1,i] - TTpath_ap[1,ap+1])./TTpath_ap[1,ap+1]
end
...
tau_real = TT_path[2:end-1]./Yv[:, ap+1]
...
for i = 1: length(Welfare)
   Epsilon2[i] = exp((Welfare[i] - Welfare[ap+1]))-1
end
