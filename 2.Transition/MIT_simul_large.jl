...
savefig(p3, "Welfare_optimal_large.png")

using LaTeXStrings
p4= plot(tauopt,100 .*Epsilon,label="Consumption Equivalent", linewidth = 2, dpi=200, color = :blue)
#p4= plot!(tau, Welfare_steady ,label="Welfare_ss", linewidth = 2, dpi=200,linestyle =:dash, legend=:bottomleft)
p4 = plot!([tauopt[ind],0.08], seriestype="vline", xticks = ([tau[ind],0.08],["\$ \\tau^{tr} = 0.0845 \$", "\$ \\tau^{p} = 0.08\$"]), linewidth = 1, dpi=200,linestyle =:dash , legend=:bottomleft, label="")
...
Welfare_low = data["Welfare"]
ind_low = data["ind"]
ind_steady_low = data["ind_steady"]
Welfare_steady_low = data["Welfare_steady"]

using LaTeXStrings
p5= plot(tauopt,Welfare,label="Welfare_transition_large", linewidth = 2, dpi=200, color = :blue)
p5= plot!(tauopt, Welfare_low ,label="Welfare_transition_low", linewidth = 2, dpi=200,linestyle =:dash, legend=:bottomleft)
p5 = plot!([tauopt[ind_low], tauopt[ind]], seriestype="vline", xticks = ([tauopt[ind_low], tauopt[ind]],["\$ \\tau^{tr_{low}} = 0.064 \$", "\$ \\quad \\quad \\quad\\tau^{tr_{large}} = 0.0845 \$"]), linewidth = 1, dpi=200,linestyle =:dash ,  label="")
savefig(p5, "Welfare_optimal_low_large.png")

Epsilon_low= ones(length(Welfare_low))
for i = 1: length(Welfare)
    Epsilon_low[i] = exp((1- β)*(Welfare_low[i] - Welfare_low[ind_low])) -1
end

using LaTeXStrings
p5= plot(tauopt,100 .*Epsilon,label="Consumption Equivalent Large", linewidth = 2, dpi=200, color = :blue)
p5= plot!(tauopt, 100 .*Epsilon_low ,label="Consumption Equivalent Low", linewidth = 2, dpi=200,linestyle =:dash, legend=:bottomleft)
p5 = plot!([tauopt[ind_low], tauopt[ind]], seriestype="vline", xticks = ([tauopt[ind_low], tauopt[ind]],["\$ \\tau^{tr_{low}} = 0.064 \$", "\$ \\quad \\quad \\quad\\tau^{tr_{large}} = 0.0845 \$"]), linewidth = 1, dpi=200,linestyle =:dash ,  label="")
savefig(p5, "Welfare_optimal_low_large.png")
