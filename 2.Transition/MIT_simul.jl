...
for i in eachindex(τls)

    # global AA0,pol0,Aggs0 = AiyagariEGM(K0,ns,Transv,states,amin,amax,curv,β,α,δ,γ,TT[i],na)
    # polA_ss[:,i],polC_ss[:,i],D_ss[:,i],A[i],Aggs_ss,Mat[:,:,i],upc[:,i],Ls[i],Ulevel[:,i] = steady(K0,pol0,AA0,R,w,TT[i],τls,1e-4)
    global AA0,pol0,Aggs0 = AiyagariEGM(K0,ns,Transv,states,amin,amax,curv,β,α,δ,γ,TT,τls[i],na)
    polA_ss[:,i],polC_ss[:,i],D_ss[:,i],A[i],Aggs_ss,Mat[:,:,i],upc[:,i],Ls[i],Ulevel[:,i] = steady(K0,pol0,AA0,R,w,TT,τls[i],1e-4)
...
using LaTeXStrings
p4= plot(tauopt,100 .*Epsilon,label="Consumption Equivalent", linewidth = 2, dpi=200, legend=:bottomleft,color = :blue)
#p4= plot!(tau, Welfare_steady ,label="Welfare_ss", linewidth = 2, dpi=200, legend=:bottomleft)
p4 = plot!([tauopt[ind_low], tauopt[ind],0.08, tauopt[ind_large]],xticks = ([tauopt[ind_low], tauopt[ind], 0.08, tauopt[ind_large]],["\$ \\tau^{low} = 6.4 \$","\$ \\tau^{c} = 7.8 \\quad \\quad \$","\$\\quad \\quad \\quad \\tau^{p}_{\\infty} = 8 \$", "\$ \\quad \\quad \\quad\\tau^{high} = 8.45 \$"]), seriestype="vline", linewidth = 1, dpi=200,linestyle =:dash ,  label="")
